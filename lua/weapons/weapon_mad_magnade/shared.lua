...
SWEP.Primary.Ammo			= "ammo_stiknade"
...
function SWEP:Think()
	...
	if (self.Primed == 1 and not self.Owner:KeyDown(IN_ATTACK) and self.PrimaryThrow) then
		if self.Throw < CurTime() then
			...
			timer.Simple(0.15, function()	if not self.Owner then return end self:ThrowStickyGrenade() self.Owner:ViewPunch(Angle(math.Rand(1, 2), math.Rand(0, 0), math.Rand(0, 0))) end)
		end
	end
	...
	if (self.Primed == 1 and not self.Owner:KeyDown(IN_ATTACK2) and not self.PrimaryThrow) then
		if self.Throw < CurTime() then
			...
			timer.Simple(0.15, function() if not IsValid(self.Owner) then return end self:ThrowGrenade() self.Owner:ViewPunch(Angle(math.Rand(1, 2), math.Rand(0, 0), math.Rand(0, 0))) end)
		end
	end
end
